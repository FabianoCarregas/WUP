...
def run(email, password, url):
    ...
    driver.find_element(By.CLASS_NAME, 'header-item-link--careers').click()
    time.sleep(1)
    ...
    driver.find_element(By.ID, 'headerBusca-campoBusca').send_keys('Conhecendo novas pessoas')
    ...
    time.sleep(2)
    driver.get('https://cursos.aluralingua.com.br/course/conhecendo-novas-pessoas-em-ingles/task/1550')
    time.sleep(2)
    ...
    driver.find_element(By.CLASS_NAME, 'task-actions-button-next').click()
    driver.find_element(By.CLASS_NAME, 'task-actions-button-next').click()
    driver.find_element('xpath', '/html/body/section/section/div/main/section[1]/div/section[1]/div/div/p[2]/input').send_keys('Beautiful X)')
    time.sleep(4)

    # finishing
    driver.get(e_url)
    driver.find_element(By.CLASS_NAME, 'headerBusca-icon-svg').click()
    driver.find_element(By.ID, 'headerBusca-campoBusca').send_keys('>>>>>>>- ALURA-LINGUA WARMED UP SUCCESSFULLY -<<<<<<<  :)')
